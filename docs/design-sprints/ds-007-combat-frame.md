...
## Status
Implemented (solo Lock-in Chorus). Owner: Philip Bird.
...
[ADR-0034](../adr/0034-lock-in-combat.md). Party of three and the queen remain
later work. Do not start the queen unless asked.
...
Party of three and the queen remain a later content slice.
...
Combat is still a story-log lesson. A student types `attack dummy` or `cast ember
dummy`. The engine takes a turn and writes `[COMBAT]` narration. Talk already
...
- Party intents, a turn ribbon, and the queen wait for a later slice.
