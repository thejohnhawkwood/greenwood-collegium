...
19. Open `https://<service>.onrender.com/health/ready` — expect HTTP 503. After Ticket 008 a successful database ping drops `database unwired`; `content absent` remains. After Ticket 009 the public page requires sign-in. Use `ADMIN_BOOTSTRAP_TOKEN` from the Render dashboard only; never paste it into Git or chat.
