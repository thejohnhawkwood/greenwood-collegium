...
- Illustrations follow `GREENWOOD_ART_DIRECTION.md` and ADR-0041. The locked style master is the ragged-boundary mouse plate: heavy-hooks ink, shadow-depth colour, and a frayed edge. A smooth closed outline fails. Do not replace production files in `public/art/` until a named revision is approved and the task names the surface. The living frame is presentation only. It must not hide command input or invent game text. Presence plates come from the current room only. Conversation overlays must not keep an NPC on the painting after that NPC leaves the scene.
...
- CombatStage renders `play-state.encounter` as a dimmed modal over the play area: both combatants, health, focus, the lock clock, the latest combat narration, and buttons that send the projected commands. The modal blocks the map, inventory, quests, and compass. The command box stays available. Overlay FX decorate the latest `combat.action_resolved` event and current foe vitals. They do not invent damage, focus spend, or legal moves. PrimerStage renders `play-state.primer` only: one School leaf at a time, with the projected nodes. Clicking a legal node sends `ink` and that node's name. Presence may send `duel <name>`. The world map fills the window. Explored rooms are named on the chart and say how to travel. The current room says you are here. Fog cells stay untitled. A click sends `travel <title>` for an explored room; the server refusal stays on the map. The explored list sends the same command.
