...
- CombatStage renders `play-state.encounter` as a dimmed modal over the play area: both combatants, health, focus, the lock clock, the server read line when present, the latest combat narration, and buttons that send the projected commands. The modal blocks the map, inventory, quests, and compass. The command box stays available. Overlay FX decorate the latest `combat.action_resolved` event and current foe vitals. A lunge, flinch, cast, guard, retreat, or ward follows that same result. They do not invent damage, focus spend, or legal moves. PrimerStage renders `play-state.primer` only: one School leaf at a time, with the projected nodes. Clicking a legal node sends `ink` and that node's name. The quest journal fills the window and shows the projected current step and reward. A duel challenge is a window from `play-state.conversation`; Accept and Decline send `duel accept` and `duel decline`. An outgoing ask is `play-state.duelAsk`. The command dock stays above those windows and shows the current quest step. Bag, Primer, and quest overlays close on Escape. Presence may send `duel <name>`. The world map fills the window. Explored rooms are named on the chart and say how to travel. The current room says you are here. A quest destination keeps that name and adds Quest. Fog cells stay untitled; a quest in fog says Quest and does not name the room. A click sends `travel <title>` for an explored room; the server refusal stays on the map. The explored list sends the same command.
