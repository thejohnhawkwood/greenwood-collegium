...
- Ticket 015 owns Arrival, `help`, `quests`, and `spells`. Handlers do not invent Porter speech or award XP twice. Persist `defeatedSpawnIds` with the Collegian. Persist Primer leaves and pending 1/2/3 offers (`known_spells`, `pending_primer_choices`, `primer_awarded_levels`). Persist personal quest-reward loot the same way as combat loot. The client must not invent Primer cards. Author spine: `docs/content/STORY.md`. Storylines: `docs/content/quests/`.
