...
- Process logs may include command verb, status, duration, and connected count. Never log raw command text, `say` lines, tickets, cookies, or `DATABASE_URL`. Teacher roster HTTP may show room chat. The 30-client load simulation stays on localhost.
