...
- Ticket 013 owns combat command dispatch. Socket handlers do not invent combat text or damage. The gateway owns the lock-in `setTimeout` and calls `handleCombatExpire`; it does not invent defend or flee text. After a party command it rearms every living member.
