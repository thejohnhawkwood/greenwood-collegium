...
- ADR-0048: `college-raider` is an enemy template with no placement. A defense mints it at the three gates at runtime, so its stats stay in JSON. `raiders-token` is its loot. Do not add a placement for it. `raid-captain` is the same kind of template: one party foe (`minParty` 3) minted at Lantern Court for that night, with no placement.
