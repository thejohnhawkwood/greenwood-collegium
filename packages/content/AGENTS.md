...
- Ticket 015 owns quest templates. Arrival at the Collegium is the first quest. First/second/third-lessons are the college loop. `defeat` and `cast` objectives are legal. Story spine: `docs/content/STORY.md`. Storylines: `docs/content/quests/`. Play/author: `docs/content/adventures.md`.
...
- `starterPerCharacter: true` on a placement is a per-Collegian recipe, not one shared instance. The Arrival copper key and the personal Field Primer use this. The moss-bound primer does not.
