...
- Ticket 015 owns Arrival at the Collegium, `help`, `quests`, `spells`, and one-time quest rewards. Join auto-look does not complete the look objective. Character lessons raise max health and focus. Investigation quests may award a personal item. They do not ink combat leaves. Author spine: `docs/content/STORY.md`. Storylines: `docs/content/quests/`.
- The Field Primer (ADR-0039) owns known leaves, ranks 1–5, pending three-choice offers, and typed `1` / `2` / `3` when a conversation is closed. `createPlayState` projects those offers as `play-state.primer` cards. College 1–5 is mentor-and-dummy. Lessons 6–20 open the Primer on the level. Dummy kills do not grant a lesson.
