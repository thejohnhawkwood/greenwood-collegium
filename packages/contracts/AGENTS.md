...
- Ticket 013 owns combat start, turn, action, end, and `progress.experience_gained`. DS-007 / ADR-0034 adds `combat.ended` outcome `fled`, action verbs `defend` / `flee`, `combat.started` enemy focus, and optional `play-state.encounter`. Optional `combat.turn_started` `lockNarration` is server-authored; the client must not invent it. Optional `combat.action_resolved` `readNote` and `play-state.encounter.read` are server-authored; the client must not invent them.
